(function(ns){
  ...
  ns.REFRESH_VUE = function refreshVue (editable) {
    ...
    var child = el.querySelector('[aem-comp]');
    var name = child.getAttribute('aem-comp');
    win.Vue.newInstance(name, {
      data: function () {
        return {
          title: ''
        }
    ...
    if (vueInstance) {
      ...
      vueInstance.$nextTick(function () {
        ...
        children.forEach(function (childEditable) {
          console.log("refreshing childEditable");
          console.log(childEditable)
